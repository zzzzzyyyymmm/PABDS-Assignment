...
output_path = 'fused_dropoff_green.parquet'  # 新的输出文件路径
...
def perform_data_fusion(df_taxi, gdf_zones, gdf_osm_pois):
    ...
    # 将出租车数据与区域数据进行合并，获取每个行程的下车点信息
    merged_taxi_zones = df_taxi.merge(gdf_zones, left_on='DOlocationID', right_on='LocationID', how='inner')
    ...
    # 空间连接：找到每个出租车下车区域附近的 POI
    gdf_taxi_with_pois = gpd.sjoin(
        gdf_poi_filtered,
        gdf_taxi_trips.drop_duplicates(subset=['DOlocationID']),
        how='inner',
        predicate='within'
    )
    ...
    print(gdf_taxi_with_pois[['name', 'fclass', 'category', 'longitude', 'latitude', 'DOlocationID']].head())

    return gdf_taxi_with_pois


def analyze_dropoff_poi_types(df):
    """
    统计下车点POI的类别数量和占比。
    """
    print("\n--- 开始统计下车点POI类型 ---")
