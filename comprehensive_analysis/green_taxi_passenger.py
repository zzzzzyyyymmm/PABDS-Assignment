...
def load_zone_centroids():
    ...
    if not os.path.exists(NYC_ZONES_PATH):
        print(f"警告: 找不到文件 {NYC_ZONES_PATH}，无法计算距离。")
        return None
    try:
...
def remove_outliers(df, column, threshold=0.98):  # 降低至 98%
    """移除指定列的异常值（基于分位数）"""
...
def prepare_prediction_data(df, service_name, centroids=None, anomaly_df=None):
    ...
    df['thermal_comfort'] = df['thermal_comfort'].fillna(df['thermal_comfort'].mean()) if 'thermal_comfort' in df else None
    df['solar_radiation'] = df['solar_radiation'].fillna(df['solar_radiation'].mean()) if 'solar_radiation' in df else None
    ...
    if centroids and 'PULocationID' in df and 'DOLocationID' in df:
        ...
        print(f"DOLocationID 与 centroids 交集比例: {len(common_ids) / len(df['DOLocationID'].unique()) if df['DOLocationID'].nunique() > 0 else 0:.2%}")
    ...
    # 先计算 trip_duration
    df['trip_duration'] = (pd.to_datetime(df[dropoff_col], errors='coerce') - df[pickup_col]).dt.total_seconds() / 60
    df = df.dropna(subset=['trip_duration'])

    # 仅对 fare_amount 移除异常值，保留 trip_duration 完整分布
    ...
    features = ['time_period', 'precipitation', 'thermal_comfort', 'solar_radiation', 'wind_dynamics', 'snow_accumulation', 'pickup_category', 'dropoff_category', 'trip_distance', 'passenger_count', 'payment_type', 'RatecodeID', 'approx_distance', 'hour']
    X = df[features]
    y_duration = df['trip_duration']
    y_fare = df['fare_amount']

    return X, y_duration, y_fare

def build_and_evaluate_model(X, y, model_type, cv=3):
    """构建模型并进行交叉验证"""
    categorical_features = [col for col in X.columns if X[col].dtype == 'object' or col in ['time_period', 'pickup_category', 'dropoff_category', 'payment_type', 'RatecodeID']]
    ...
    if model_type == 'random_forest':
        ...
        model = Pipeline(steps=[('preprocessor', preprocessor),
                                ('regressor', GradientBoostingRegressor(n_estimators=200, max_depth=12, learning_rate=0.05, random_state=42))])

    scores = cross_val_score(model, X, y, cv=cv, scoring='r2', n_jobs=-1)
    print(f"\n{model_type.upper()} Model - Cross-validation R² scores: {scores}")
...
def main():
    ...
    X, y_duration, y_fare = prepare_prediction_data(final_df, service, centroids, anomaly_df)

    if X is None or y_duration is None or y_fare is None or X.empty:
        print(f"警告: {service} 数据为空或无效，跳过模型训练。")
        return

    print(f"\n--- 预测 {service} 订单时间 (分钟) ---")
    rf_duration_model = build_and_evaluate_model(X, y_duration, 'random_forest')
    gb_duration_model = build_and_evaluate_model(X, y_duration, 'gradient_boosting')

    print(f"\n--- 预测 {service} 订单价格 (美元) ---")
    rf_fare_model = build_and_evaluate_model(X, y_fare, 'random_forest')
    gb_fare_model = build_and_evaluate_model(X, y_fare, 'gradient_boosting')
